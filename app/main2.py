...
class Spot:
    ...
    def update_neighbours(self, grid):
        ...
        if self.row > 0 and self.col > 0 and not grid[self.row - 1][self.col - 1].is_barrier(): # DOWN LEFT
            self.neighbours.append(grid[self.row - 1][self.col - 1])
        
        if self.row > 0 and self.col < self.total_rows - 1 and not grid[self.row - 1][self.col + 1].is_barrier(): # DOWN RIGHT
            self.neighbours.append(grid[self.row - 1][self.col + 1])
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r and not started:
                    for row in grid:
                        for spot in row:
                            if (spot.is_open or spot.is_closed()) and not spot.is_barrier() and spot != start and spot != end:
                                spot.reset()
